...
Use `opendle-pio` for each PlatformIO command. The shared tool runs the same
...
```sh
uv run --locked opendle-pio run --environment esp32dev
uv run --locked opendle-pio run --environment lolin32_lite
```
...
```sh
uv run --locked opendle-pio run --environment esp32dev_led_test --target upload
```
...
```sh
uv run --locked opendle-pio run --environment esp32dev_fast_test --target upload
```
...
```sh
uv run --locked opendle-pio run --environment esp32dev_calibration --target upload
...
```sh
uv run --locked opendle-pio run --environment esp32dev --target upload --upload-port /dev/cu.usbserial-0001
uv run --locked opendle-pio device monitor --port /dev/cu.usbserial-0001 --baud 115200
```

For the WEMOS, connect the external USB-C panel cable to a computer. Then find
its port with `uv run --locked opendle-pio device list` and use the
